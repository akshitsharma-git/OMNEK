...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-6">
      ...
      <div className="signup-container w-full max-w-md relative z-10">
        {/* Logo Header */}
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">
            Join the OMNEK community today
          </p>
        </div>
          ...
          <form onSubmit={handleSignup} className="space-y-5">
              ...
              {password && (
                <div className="mt-2 space-y-1">
                  <div className="flex gap-1">
                    <div className={`strength-bar h-1 flex-1 rounded ${password.length >= 8 ? 'bg-green-500' : 'bg-slate-700'}`}></div>
                    <div className={`strength-bar h-1 flex-1 rounded ${password.length >= 10 && /[A-Z]/.test(password) ? 'bg-green-500' : 'bg-slate-700'}`}></div>
                    <div className={`strength-bar h-1 flex-1 rounded ${password.length >= 12 && /[A-Z]/.test(password) && /[0-9]/.test(password) ? 'bg-green-500' : 'bg-slate-700'}`}></div>
                  </div>
                  <p className="text-xs text-slate-400">
                    {password.length < 8 ? 'Use 8+ characters for better security' : 'Strong password!'}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-start gap-2 text-sm">
              <input 
                type="checkbox" 
                required
                className="w-4 h-4 mt-0.5 rounded border-slate-600 bg-slate-900 text-blue-600 focus:ring-2 focus:ring-blue-500/20"
              />
              <label className="text-slate-300">
                I agree to the{" "}
                <a href="#" className="link-hover text-blue-400 font-medium">Terms of Service</a>
                {" "}and{" "}
                <a href="#" className="link-hover text-blue-400 font-medium">Privacy Policy</a>
              </label>
            </div>
